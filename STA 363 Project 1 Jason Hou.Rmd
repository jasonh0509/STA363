...
For the k fold Cross Validation, Figure 2.3 shows that  k = 8 is the best k value for 10-fold Cross Validation because it provides the largest geometric mean among k values from 1-50, indicating the most ideal balance between the sensitivity and specificity of the prediction.




```{r}
knitr::kable(table(YHatOut, dengue$Y), caption= "Table 2: Confusion Matrix of Predicting Dengue Status (10 Fold CV & KNN with K = 8)  ", col = c("Not Dengue", "Dengue"))  
```

We have:

$$Sensitivity = \frac{3431}{3431+597}\approx 0.852$$

$$Specificity = \frac{615}{615+1083}\approx 0.362$$

$$Accuracy = \frac{615+3431}{5726}\approx 0.71$$
### Table 3 Comparison of Predictive Metrics of 2 Types of KNN
...
knitr::kable((correlation_tr),caption = "Correlation Matrix of Numeric Features")
```
...
```{r}
knitr::kable(table(storage_logistic$Predicted, dengue$Y), caption= "Table 3: Confusion Matrix of Predicting Dengue Status (Logistic Regerssion & 10 Fold cv)  ", col = c("Not Dengue", "Dengue"))  
```

For prediction through logistic regression, we ha
