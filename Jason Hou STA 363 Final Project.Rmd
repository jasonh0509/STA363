...
stroke_fold<-sample(stroke_pool,2090,replace = FALSE)
...
for(j in 1:10){
  ...
  kpreds_k2<-knn(stroke_trainK2[,c(1,2,3)],stroke_testK2[,c(1,2,3)],k=2,cl=stroke_trainK2$stroke)
...
ridge.modFind_Lambda <- cv.glmnet(XD[,-c(1,12)], newStroke$stroke , alpha = 0,lambda = seq(from=0, to=50,by=0.005), standardize = TRUE,family = "binomial")
```
...
```
```{r smotenc}
#SMOTE_NC(stroke,"stroke",perc_maj = 20)
```
...
```{r}
ridge.final<-glmnet(XD[,-c(1,12)], newStroke$stroke , alpha = 0,lambda = 0.005	, standardize = TRUE,family = "binomial")
pred<-predict(ridge.final,newx=XD[,-c(1,12)],s=0.005	,type = "response")
...
```{r cv lasso}
set.seed(114514)
lasso.modFind_Lambda <- cv.glmnet(XD[,-c(1,12)], newStroke$stroke , alpha = 1,lambda = seq(from=0, to=50,by=0.005), standardize = TRUE,family = "binomial")
...
```{r}
lasso.final<-glmnet(XD[,-c(1,12)], newStroke$stroke , alpha = 1,lambda = 0.005	, standardize = TRUE,family = "binomial")
pred_lasso<-predict(lasso.final,newx=XD[,-c(1,12)],s=0.005,type = "response")
