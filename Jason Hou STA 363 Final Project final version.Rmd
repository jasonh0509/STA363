...
ggplot(data=test,aes(x=bmi))+
  geom_boxplot()+
  ggtitle("Figure 2.3 Distribution of BMI")
```
...
We can see from Figure 2.6 that the distribution of average glucose level of patients is rightly skewed.


```{r}
ggplot(data=test,aes(x=age,y=bmi))+
  geom_point()+
  ggtitle("Figure 2.7 Scatterplot of Patients' Age vs BMI ")+
  geom_smooth()
```

We can see from Figure 2.7 that patients' age and their BMI may have correlation.

```{r}
ggplot(data=test,aes(x=avg_glucose_level,y=bmi))+
  geom_point()+
  geom_smooth()+
  ggtitle("Figure 2.8 Scatterplot ")
```

We can see from Figure 2.8 that patients' BMI and their average blood glucose level may have correlation.

```{r}
ggplot(data=test,aes(x=age,y=bmi,color=stroke))+geom_point()+ggtitle("Figure 2.9 Age vs BMI Plot Respect to Stroke Status")
```
...
##### Table 3.3.2 Predictive Metrics (kNN)
...
corrplot(M, method= "number",type = "upper",title="Figure 4.1 Correlation Plot of Numeric Variables in Stroke Data Set ",mar = c(0,0,2,0))
```

We can see from the Figure 4.1 that correlations present between the numeric features in the data set, indicating that multicolinearity exists across features, therefore utilizing penalized regression models is appropriate for the data set.

Therefore, we decided to implement penalized regression techniques, along with logistic regression, to build the second classifier that not only utilizes both numeric and categorical features on making prediction, but also mitigate the impact of multicolinearity (correlations between features) in the prediction.
...
$$log(\frac{\boldsymbol{Y}}{1-\boldsymbol{Y}})=\boldsymbol{X_D\beta+\epsilon}$$ , where XD is our design matrix that consists of all the features remained in the data set after data cleaning.
...
##### Table 4.2.3 Predictive Metrics of Ridge Regression Model (Threshold unadjusted)
...
,caption = "Table 4.2.4 Threshold With Ideally Balanced Sensitivity and Specificity and Largest Geometric Mean ")
```
...
```{r ridge mtx2}
knitr::kable(table("Prediction" = pred_ridge_Y,"Actual"= test$stroke),caption = "Table 4.3.1 Confusion Matrix for Prediction of Stroke Via Ridge(threshold adjusted")
```

From Table 4.3.1,we have;

##### Table 4.3.2 Predictive Metrics of Ridge Regression Model(Threshold Adjusted)
...
The ridge regression model enabled us to predict more accurately than kNN on identifying people with higher stroke risk. However, the decreased CER on prediction would lead to more incorrect prediction on people' stroke risk. Therefore, further techniques may be needed for mitigating this issue and developing better classifier on stroke risk prediction.
...
$$Specificity=\frac{4510}{4510+190}\approx0.960$$ $$Accuracy=\frac{4511+785}{5875}\approx0.917$$
...
$$Accuracy=\frac{4532+750}{5875}\approx0.920$$ $$CER=1-Accuracy=1-0.920=0.08$$
