...
Table 2.1 and Figure 2.1 show that 201 rows of missing values were found from the data set, which were only present in the column "bmi". According to Table 2.2, there are 161 patients who suffered from stroke and 40 patients who did not suffer from stroke in patients with their BMI value missing. We decide to remove these rows with missing values in BMI.
...
```{r remove BMI larger than 75}
test<-test%>%
  subset(bmi<75)
```

We can see from Figure 2.3 that extreme values of BMI exist in the data set. We decided to remove 3 observations with BMI larger than 75 since it might be unreasonable for people to have such high BMI.
...
corrplot(M, method= "number",type = "upper",title="Figure 2.4 Correlation Plot of Numeric Variables in Stroke Data Set ",mar = c(0,0,2,0))
```

We can see from the Figure 2.4 that correlations present between the numeric features in the data set, indicating that multicolinearity exists across features, therefore utilizing penalized regression models(ridge, lasso, elastic net) were appropriate for the data set.
...
$$Sensitivity=\frac{505}{670+505}\approx0.514$$
$$Specificity=\frac{4325}{4325+372}\approx0.928$$
$$Accuracy=\frac{4325+505}{5872}\approx0.823$$
$$CER=1-Accuracy=1-0.823=0.177$$


According to the result obtained from 10-fold Cross Validation, kNN allowed us to reach a sensitivity of 0.514 and a specificity of 0.928, and an overall accuracy of 0.823 on predicting for patients in the stroke data set on whether each of them are more likely or less likely to have stroke. Despite the accuracy of 0.83 that indicates we made accurate predictions on 83% of the patients, the sensitivity of our prediction revealed that we only predicted accurately on 51.4% of the patients regarding their risk of suffering from stroke.
...
$$Sensitivity=\frac{430}{745+430}\approx0.366$$
