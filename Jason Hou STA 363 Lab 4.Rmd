...
We should have 34 rows in fold of the penguins data. Some folds will not have exactly the same number of rows since 333 is not fully divisible by 10.
...
knitr::kable(which(foldsSmall == 2),caption = "Rows Went to Fold2")
```
...
knitr::kable(fold2,caption = "Fold2 Rows")
```
...
knitr::kable(fold10,caption = "Fold10 Rows")
...
```{r fold 1}
f<-1
infold1<-which(foldsSmall==1)
knitr::kable(infold1,caption = "Rows Went to Fold1")
```

```{r}
newTrainF1<-penguinsSmall[-which(foldsSmall == 1),]
newTestF1<-penguinsSmall[which(foldsSmall == 1),]

K_predsFold1<-knn(newTrainF1[,c(3,5)],newTestF1[,c(3,5)],cl = newTrainF1$sex,k=5)

storageSmall$YHat[infold1]<-as.character(K_predsFold1)
print(storageSmall)
```

Row 1,7, 12 and 14 are filled in. They are filled in because they are the rows assigned to fold1 during the sampling process. When we use fold 1 as testing data set, the sex of penguins recorded in these rows were predicted and filled in.
...
```{r fold 2 prediction, echo=TRUE}
g<-2
infold2<-which(foldsSmall == 2)
knitr::kable(infold2,caption = "Rows Went to Fold2")
```
...
storageSmall$YHat[infold2]<-as.character(K_predsFold2)
print(storageSmall)
```

Row 2, 13,15,19 are filled. It is filled in because they are assigned to fold 2 during the sampling process. When we use fold2 as the testing data set,the sex of penguin recorded in these rows were predicted and filled in.
...
knitr::kable(table(storagePenguin$Yhat, penguins$sex), caption= "Table 1: Confusion Matrix of Male/Female Penguin Prediction(k=5)", col = c("True Female", "True Male"))
```
...
$$Sensitivity=\frac{129}{39+129}\approx0.768$$

$$Specificity=\frac{118}{118+47}\approx0.715$$

$$Geometric Mean=\sqrt{Sensitivity*Specificity}=\sqrt{0.768*0.715}\approx0.740$$

From the confusion matrix, we calculated that the sensitivity of the prediction is about 0.768, while the specificity is 0.715. The geometric mean of sensitivity and specificity for 10-fold CV with 5 nearest neighborhood is 0.740.



```{r}
storage<-data.frame("k"=rep(NA,10),"GMean"=rep(NA,10))

true1<-which(penguinTest$sex=="male")
