---
title: "STA 363 Lab 3"
author: "Jason Hou"
date: '2022-09-09'
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
```

```{r library load, include=FALSE}
library(palmerpenguins)
library(tidyverse)
library(broom)
library(janitor)
library(ggplot2)
library(class)
data("penguins")
```

```{r remove NA}
penguins <- na.omit(penguins)
```


## Question 1

*What problems occur when we split the training data set into two data sets, especially with a data set this small?*

One of the problem that could occur when we split the training data set into two data set is the sample size reduction, meaning that we have fewer amount of data we can utilize to train the model,which could lead to the reduction in accuracy in the trained model, when the data set is small, these issues can grow into a more severe extent.

```{r seed test}
set.seed(100)
trainRows <- sample(1:333, 200)
```


## Question 2

*Create a code chunk. Use the code sample(1:10, 2) to print out 2 random numbers between 1 and 10. Hit play on the chunk. What numbers do you get? Now, hit play again. What numbers do you have now?*


```{r random sampling from 1 to 10}
sample_from_1_to_10<-sample(1:10, 2)
knitr::kable(sample_from_1_to_10,col.names = "Two Numbers Selected From 1 to 10")
```

Every time when I run the code chunk above, the numbers I will get is different.

## Question 3

*Now, add the line set.seed(435) to the beginning of your code chunk from the previous question (meaning this line needs to come before the sample command.) You will note that I used 435, but you can use literally any positive integer you want as your random seed. Hit play on the chunk. What numbers do you get? Now, hit play again. What numbers do you have now?*

```{r draw 2 from 10}
set.seed(435)
Two_numbers_selected<-sample(1:10, 2)
knitr::kable(Two_numbers_selected,col.names = "Two Numbers Selected")
```

After setting the seed before running the sample function, the number get from sampling become a constant pair, which is 6 and 10.

## Question 4

*Set a random seed of 367. Show the code you would need to sample 5 random numbers between 5 and 678.*

```{r draw 5 numbers, echo=TRUE}
set.seed(367)
#sample(5:678,5)

sample_num<-sample(5:678,5)
knitr::kable(sample_num,col.names = "Sample Number Selected")
```


```{r trainRows}
set.seed(100)
trainRows <- sample(1:333, 200)
```

## Question 5

*Set a random seed of 245. Show the code you would need to sample 5 random numbers between 5 and 200, and store the results as trainPractice. Which 5 random numbers did you select?*

```{r draw 5 numbers from 200, echo=TRUE}
set.seed(245)
trainPractice<-sample(5:200,5)
knitr::kable(trainPractice,col.names = "Number Selected")

```

The 5 random numbers that are selected are 86,45,62,58,65.

## Question 6

*Using the rows indicated in the trainPractice vector you created in Question 4, create a data frame (data set) called trainPracticeData by selecting only the rows from the penguin data that are indicated in trainPractice. Print out the result by typing trainPracticeData in a chunk and pressing play.*


```{r trainPractice}
trainPenguins <- penguins[trainRows,]
```

```{r}
trainPracticeData <-penguins[trainPractice,]
knitr::kable(trainPracticeData)

```


```{r test}
testPenguins<-penguins[-trainRows,]
```

```{r remove data frame not needed}
rm(trainRows,trainPractice,trainPracticeData)
```

