...
```{r 8020 again with k=46}
split8020<-data.frame("Predictions"=rep(NA,5726))
k_preds_split_best<-knn(manual_train[,c(2,3,8,9)],manual_test[,c(2,3,8,9)],k=46,cl=manual_train$Y)
split8020$Predictions<-as.numeric(as.character(k_preds_split_best))
knitr::kable(table(k_preds_split_best, dengue$Y), caption= "Table 1: Confusion Matrix ", col = c("True Not", "True Dengue"))
```
