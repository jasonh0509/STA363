...
```{r}
model3<-glm(formula = Y ~ Sex + Vomiting + Temp + Age*Weight + BMI, family = binomial, 
    data = dengue)
model3
```

```{r}
modelT<-glm(data=logisticTrain,family = binomial,Y~Sex+Vomiting+Temp+Age*Weight+BMI+Rash+Hepatomegaly+Flush)
modelT
```
