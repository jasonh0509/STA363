...
```{r k fold}
dengueNum<-nrow(knnActual)
set.seed(363663)
pool<-rep(1:10,573)

foldDengue<-sample(pool,5726,replace = FALSE)

storeK<-data.frame("K"=rep(NA,50))


for (K in 1:50) {
  #Store k
  storeK$K[K]<-K
  #Store predictions from inner
  store_inner<-data.frame("Yhat"= rep(NA,5726))
  #store_inner<-store_inner%>%
    #mutate(YHat=as.character(YHat))


  
  for(p in 1:10){
    infold<-which(foldDengue == p)
    dengueTrain<-knnActual[-infold,]
    dengueTest<-knnActual[infold,]
    
    k_preds_dengue<-knn(dengueTrain[,c(1:11)],dengueTest[,c(1:11)],cl=dengueTrain$Y,k=K)
    
    store_inner$YHat[infold]<-as.numeric(as.character(k_preds_dengue))
