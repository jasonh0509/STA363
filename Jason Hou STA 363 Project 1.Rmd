...
```{r Vomit and DayDisease, eval=FALSE, include=FALSE}
dengueForCheck%>%
...
```{r DayDisease Abdominal Pain, eval=FALSE, include=FALSE}
...
```{r day and Muco, eval=FALSE, include=FALSE}
dengueForCheck<-dengueForCheck%>%
...
day_Muco
```
```{r Temp abdo, eval=FALSE, include=FALSE}
dengueForCheck<-dengueForCheck%>%
  mutate(Abdo=as.factor(dengueForCheck$Abdo))
...
```{r exxploring correlation}
correlation<-cor(dengue[,c(2,8:11)])
...
```{r try model}
glm(data=dengue, family = binomial, Y~Sex+Vomiting+Temp)
```
