...
```{r lasso final}
ridge.mod_Final <- glmnet(XD[,-1], vrboCleaned$price , alpha = 1 , lambda = 0.75 , standardize = TRUE)
Betas_lasso<-as.numeric(coefficients(ridge.mod_Final))
lasso_coef<-data.frame("Coefficients" = Betas_ridge)
...
storageElastic<-data.frame("Alpha" = rep(NA,length(alphaseq)), "Lambda" = rep(NA,length(alphaseq)), "RMSE" = rep(NA,length(alphaseq)))
...
for(i in alphaseq){
  ...
  storageElastic$Lambda[a]<-cvOUt$lambda.min
  storageElastic$RMSE[a]<-sqrt((min(cvOUt$cvm)))
...
```{r smallest MSE}
storageElastic[which.min(storageElastic$RMSE),]
```
...
knitr::kable(elastic_coef,caption = "Coefficients of Model using Elastic Net")
```
