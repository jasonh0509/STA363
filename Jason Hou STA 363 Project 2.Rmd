...
Renting homes and apartments that belong to local individuals as a place to live has gradually become a new way of finding a place to stay during vacations besides the traditional hotel room, as it provides a cozy, home-like feeling and provides opportunities for people to experience their traveling destinations from a new aspect. Therefore, it is important for platforms like VRBO, which allows individuals to post their homes and apartments for short-term rent, to find methods that help to promote the renting of properties and benefit the platform in long run. The primary goal of this project is to build a model that can be used to predict the price of a rental based on the features of the property, therefore determining the appropriate price property owners should set for their rental properties posted on VRBO to maximize the chance of being rented by travelers. Three regression methods, ridge regression, lasso regression, and elastic net regression were implemented to build this model for predicting the appropriate price of a VRBO rental property. We found the model built through elastic net regression has a better performance in predicting the price of a VRBO rental property, while improvements on the regression methods used can still be performed to further improve the predictive accuracy of the model.

## Part 1: Introduction of Data

We are working on the data set that records 1561 VRBO rental properties. The data set contains information about attributes of the property that might impact the rental price of the properties, including the overall satisfaction score and reviews from customers, the capacity of the properties, location, percentage of rental property nearby, and traffic-related attributes. The goal of our client is to use the data to build a model for predicting the price of a rental, which can be used to help the client to set appropriate prices for new rental properties that make it more likely for them to be rented out.
...
We can see from Table 2.1 that 138 rows with missing values were found in the data set. They are removed from the data set. The 1423 observations were used for the following steps. We can also see that the distribution of the price of VRBO rental properties is rightly skewed.
...
We decide to utilize a correlation plot to explore if correlated features exist in the data set. A correlation plot for numeric features in the data set is built to explore if there are potential correlations between features.
...
$$RSS + \lambda \boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}=(\boldsymbol{Y-X_\mathbf{D}\hat{\beta}})^T(\boldsymbol{Y-X_\mathbf{D}\hat{\beta}})+\lambda\boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}$$ To do this, we need to find the value of tuning parameter $\lambda$ that provides the most ideal shrinkage. We fit models with $\lambda$ value from 0 to 50, increment by 0.05 for each time to find the ideal value of $\lambda$. We choose to use the root mean square error(RMSE) to assess the predictive accuracy of each model that uses different values of $\lambda$(tuning parameter). The RMSE is defined as the square root of the mean squared error, which, in other words, the estimated standard deviation of residual of the model.The regression model is said to have less error if the data points are closer to the model, which lead to a smaller RMSE that indicates a better predictive accuracy. The RMSE allows us to assess how well the model fits the observed data points, so that we can determine the predictive accuracy of the model.We use 10 fold Cross Validation to fit the model with to calculate the RMSE and then create a graph of change in RMSE in response of $\lambda$,and select the corresponding $\lambda$ value which provides the ideal extent of shrinkage, as well as balancing bias and variance. The features in the design matrix are standardized when performing 10 fold Cross Validation.
...
We can see from Figure 3.2 that the smallest RMSE appeared when the model is using $\lambda = 13.1$ as the tuning parameter for the ridge regression. Thus, we chooses this value of $\lambda$ as the $\lambda$ for our final ridge regression model and fit the regression model. The coefficient is shown in the Table 3.2.

```{r}
knitr::kable(CV_result_ridge[smallestRidge_RMSE,],caption = "Table 3.1 RMSE of Model with Chosen Lambda(Ridge)")
...
knitr::kable(ridge_coef,caption = "Table 3.2 Coefficients of Model using Ridge Regression")

```

```{r}
knitr::kable(sum(ridge_coef$Coefficients!=0)-1,caption = "Table 3.3 Number of Features in Final Ridge Model")

```

We can see from Table 3.1 and 3.3 that the RMSE of the final ridge regression model has 61 features with an RMSE of 60.17.

## Part 4: Lasso

We then try to use the lasso regression to fit the model for predicting prices of VRBO rental properties since it has the effect of both shrinkage and selection. In other words, lasso regression allows us to remove correlated features from the model to resolve the multicollinearity issue without potentially sacrificing the accuracy of the prediction from the model. Like ridge regression, we need to find the best tuning parameter that controls the selection process and generate the model with the most favorable predictive accuracy.

We use RSS, plus the penalty term $\lambda||\boldsymbol{\hat{\beta}}||_1$ for lasso regression. Our goal is the find the $\boldsymbol{\hat{\beta}}$ that minimize the following: $$RSS+\lambda||\boldsymbol{\hat{\beta}}||_1 + =(\boldsymbol{Y-X_\mathbf{D}\hat{\beta}})^T(\boldsymbol{Y-X_\mathbf{D}\hat{\beta}})+\lambda\sum|\hat{\beta_j}|$$.
To do this, we fit models for each $\lambda$ value from 0 to 50, increments by 0.05 for each time, and compare the predictive accuracy of each model to determine the ideal value of $\lambda$.
...
knitr::kable(lasso_coef,caption = "Table 4.2 Coefficients of Model using Lasso Regression")

```

```{r}
knitr::kable(sum(lasso_coef$Selecting.Status!="Removed"),caption = "Table 4.3 Number of Features in Final Lasso Model")
```

We can see from Table 4.1 and 4.3 that the final lasso model has 34 features with an RMSE of 60.36.

## Part 5: Elastic Net

Ridge regression is effective in dealing with the situation where we have more features than the number of rows, and constrain the impact of correlated features toward the model, while lasso has the advantage of shrinking and selecting features at the same time. However, ridge regression is unable to select features, and lasso regression is designed to select features instead of shrinking them, which leads to a model with limited features that potentially sacrifices predictive accuracy compared to ridge regression. Therefore, we decide to try to use the elastic net regression, which allows us to use both ridge regression and lasso regression at the same time to balance the shrinkage approach of the ridge regression and the ability of selection from lasso regression, to fit the model for predicting the prices of VRBO rental properties. To build the model with the best predictive accuracy, we need to find the ideal tuning parameters that control the extent of shrinkage and selection when fitting the model.For elastic net, we choose the $\boldsymbol{\hat{\beta}}$ that minimizes $$RSS + \lambda \sum((1-\alpha)\hat{\beta_j^2}+\alpha|\hat{\beta_j}|)$$. We use 10 fold Cross Validation fit models for each value of $\alpha$(from 0 to 1, increment by 0.01 each time) and $\lambda$(from 0 to 25, increment by 0.5 each time), and calculate the RMSE for each model.The features in the design matrix are standardized when performing 10 fold Cross Validation.
...
knitr::kable(elastic_coef,caption = "Table  5.2 Coefficients of Model using Elastic Net")
```

```{r}
knitr::kable(sum(elastic_coef$Selecting.Status!="Removed")-1,caption = "Table 5.3 Number of Features in Final Elastic Net Model")
```


We can see from Table 5.1 and 5.3 that the final elastic net model has 36 features with an RMSE of 59.34.

## Part 6: Comparison and Conclusion

We listed out the coefficient table of three regression method we used, and compare tuning parameter and predictive accuracy of the three models.
...
Table 6.2 Comparison Across Ridge Regression, Lasso Regression and Elastic Net Regression on Tuning Parameter, Predictive Accuracy Metrics, and Features(intercept removed)
...
We can see from Table 6.2 that the RMSE of model built through ridge regression, lasso regression and elastic net regression, which indicates the estimated standard deviation of residuals of the model, are 60.17, 60.36 and 59.34, respectively. The RMSE of final ridge regression model is 60.17, which is about 0.3% lower than the RMSE of model built with lasso regression.The RMSE of model built through elastic net is 1.7% lower than the model built with lasso, and 1.4% lower than the model built with ridge. We can see that the elastic net model performs better at prediction than the other two models. The model fitted with elastic net has the lowest RMSE of 59.34 (with $\alpha = 0.15$ and $\lambda = 5$) among the three models fitted model, consider the range the price varies in the data set(from 0 to 1000), this RMSE of 59.34 indicates a relatively better fit of the model to the data points, which lead to a better predictive accuracy of the model and simultaneously balances bias and variance.

While ridge regression allows us to keep all the features provided in the model, including all the features in the model could increase model's complexity, making it hard to interpret when compared with lasso and elastic net. Lasso regression allows us to completely remove correlated features instead of shrinking them, but it also imposes limitation on the number of features selected, which lead to a lower predictive accuracy compared to ridge regression and lasso regression as it shown in Table 6.2, where we can see that the elastic net regression model performs better at prediction than the other two models.

Based on the information we have obtained, we conclude that the model built with elastic net regression offers the best prediction to the price of VRBO rental properties, and we recommend the client to choose it as the model for predicting the price of a VRBO rental property. The limitation that the model has in real-world setting is that it can be computationally expensive, since the process of finding the best tuning parameters can be lengthy when the model is applied to a large data set. We also suggest that a mixed effect model could also be introduced to fit regression model, as it has the ability of addressing the random effects(variance due to difference between neighborhood blocks and districts, and variances due to the difference among each individual rental property within the block/district.
