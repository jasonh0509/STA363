...
The Residual Sum of Squares (RSS) of a model is defined as the standard deviation of observed values in the data set vs predicted values. The equation is : $$RSS=(\boldsymbol{Y-X_\mathbf{D}\widehat{\beta}})^\mathbf{T}(\boldsymbol{Y-X_\mathbf{D}\widehat{\beta}})$$

where XD is our design matrix that consist of

For ridge regression ,we will use RSS, plus the penalty term $\lambda\boldsymbol{\widehat{\beta}^ \mathbf{T}\mathbf{\widehat{\beta}}}$, which serves to constraining the regression coefficients under the presence of correlated features.

Our goal is to find the tuning parameter $\lambda$, as well as the $\boldsymbol{\beta}$ that minimizes the following:

$$RSS + \lambda \boldsymbol{\widehat{\beta}^\mathbf{T}\mathbf{\widehat{\beta}}}=(\boldsymbol{Y-X_\mathbf{D}\widehat{\beta}})^T(\boldsymbol{Y-X_\mathbf{D}\widehat{\beta}})+\lambda\boldsymbol{\widehat{\beta}^\mathbf{T}\mathbf{\widehat{\beta}}}$$

In order to find the best $\lambda$ value that provides the most appropriate shrinkage for the coefficients, we are going to minimize the values of coefficients for each $\lambda$ value from 0 to 50, increment by 0.05 for each time.We used the root mean square error(RMSE) to assess the predictive accuracy of each model that uses different values of $\lambda$(tuning parameter). The RMSE is defined as the square root of the mean squared error, which is the mean of the difference between the predicted values and observed values. The RMSE allows use to assess how well the model fits the observed data points, so that we can determine the predictive accuracy of the model. Lower RMSE indicates a higher predictive accuracy, higher RMSE, on the other hand, indicates lower predictive accuracy. 10 fold Cross Validation was used to calculate the RMSE, the features in the design matrix were standardized.
