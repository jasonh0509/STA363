...
We utilized a correlation plot to explore if correlated columns exist in the data set.Thus, we created a correlation plot for numeric features in the data set to explore if there are potential correlations present between features.
...
where XD is our design matrix that consist of all the features in the data set,these features in the design matrix were standardized..
...
$$RSS + \lambda \boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}=(\boldsymbol{Y-X_\mathbf{D}\hat{\beta}})^T(\boldsymbol{Y-X_\mathbf{D}\hat{\beta}})+\lambda\boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}$$ To do this, we need to find the value of tuning parameter $\lambda$ that provides the most ideal shrinkage on the coefficients.

We minimize the values of coefficients for each $\lambda$ value from 0 to 50, increment by 0.05 for each time.We choose to use the root mean square error(RMSE) to assess the predictive accuracy of each model that uses different values of $\lambda$(tuning parameter). The RMSE is defined as the square root of the mean squared error, which is the mean of the difference between the predicted values and observed values. The RMSE allows use to assess how well the model fits the observed data points, so that we can determine the predictive accuracy of the model. Lower RMSE indicates a higher predictive accuracy, higher RMSE, on the other hand, indicates lower predictive accuracy. We use 10 fold Cross Validation to calculate the RMSE and create a graph of change in RMSE in response of $\lambda$,and select the corresponding $\lambda$ value which provides the shrinkage that balance between variance and bias of the model.
...
We then tried to use the lasso regression to fit the model for the prediction of vrbo price since it has the effect of both shrinkage and selection. In other words, lasso regression allows us to remove correlated features from the model to resolve multicollinearity issue without sacrificing the accuracy of the prediction from the model.Like the ridge regression, we need to find the best tuning parameter that controls the selection process and generate the model with the most favorable predictive accuracy.

We use RSS, plus the penalty term $\lambda||\boldsymbol{\hat{\beta}}||$ for lasso regression. Our goal is the find the $\boldsymbol{\hat{\beta}}$ that minimize the following: $$RSS + \lambda \boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}=(\boldsymbol{Y-X_\mathbf{D}\hat{\beta}})^T(\boldsymbol{Y-X_\mathbf{D}\hat{\beta}})+\lambda\sum|\hat{\beta_j}|$$. To do this, we minimized and select the coefficients for each $\lambda$ value from 0 to 50, increments by 0.05 for each time. We use the RMSE to assess the predictive accuracy of each model with different $\lambda$, and create a graph of change in RMSE in response of the change in $\lambda$, then select this $\lambda$ as the value of tuning parameter used in final model,as it provides the shrinkage and selection that lead to the highest predictive accuracy and balance between bias and variance.
...
We can see from Figure 4.1 that the lowest RMSE appeared when the model is using $\lambda=0.75$ as the tuning parameter. Thus, we choose $\lambda = 0.75$ as the final model for the prediction of price.The coefficients of the model are listed below.
...
Ridge regression is effective on dealing with situation where we have more features than the number of rows, and constrain the impact of correlated features toward the model, while lasso has the advantage of shrinking and selecting features at the same time. However, ridge regression is unable to select features, and lasso regression is designed to select instead to shrink, which leads to model with limited features that sacrifices predictive accuracy. Therefore, we utilize the elastic net regression, which allows us to use both ridge and lasso regression at the same time to balance the shrinkage approach of ridge and ability of selection from lasso. In order to fit the model with the ideal predictive accuracy, we need to find the tuning parameters that controls the extent shrinkage and selection when fitting the model.For elastic net, we choose the $\boldsymbol{\hat{\beta}}$ that minimizes $$RSS + \lambda \sum((1-\alpha)\hat{\beta_j^2}+\alpha|\hat{\beta_j}|)$$. We minimize and select coefficients for each value of $\alpha$(from 0 to 1, increment by 0.01 each time) and $\lambda$(from 0 to 25, increment by 0.5 each time), and calculate the RMSE for each model.
...
We can see from Table 5.1 that the lowest RMSE appeared when $\alpha=0.15$ and $\lambda = 5$. Thus we choose $\alpha = 0.15$ and $\lambda = 5$ to be the value of tuning parameters used in the final model. The coefficients of the final model using elastic net regression are list below.
...
Table 6.2 Comparison Across Ridge, Lasso and Elastic Net on Tuning Parameter, Predictive Accuracy Metrics, and Features
...
We can see from Table 6.2 that the model fitted with elastic net has the lowest RMSE of 59.34 among the three models fitted for predicting the price of vrbo rentals(with $\alpha = 0.15$ and $\lambda = 5$). The RMSE of model built through elastic net is 1.7% lower than the model built with lasso, and 1.4% lower than the model built with ridge. Therefore, we conclude that the model built with elastic net regression offers the best prediction to the price of vrbo rental properties.