...
We can see from Figure 2.1 that several significant correlations are presented between features, indicating shrinkage and selection techniques are appropriate to be used on building the model.


Firstly, we use the ridge regression to fit the model, as it provides the effect of shrinkage. That is,shrinking the coefficients of correlated features, therefore limits the impacts from these features to the model to off set the negative effect induced by multicollinearity(correlated columns in data set). For this we need to find the tuning parameter that provides the best shirking effect. 


We will use the term Residual Sum of Squares (RSS), defined as the standard deviation of observed values vs predicted values, plus the penalty term $\lambda\boldsymbol{\widehat{\beta}^\mathbf{T}\mathbf{\widehat{\beta}}}$
