---
title: "Classifier for Predicting Stroke Risks"
author: "Jueshen Hou"
date: '2022-12-09'
output: 
  pdf_document:
...
originalStroke<-read.csv("https://raw.githubusercontent.com/jasonh0509/STA363/main/StrokeSet.csv")
```
...
```{r show where NA is,fig.height=5,fig.width=5,fig.align='center'}
NA_plot<-vis_miss(originalStroke)
NA_plot+ggtitle("Figure 2.1 Graph of Missing Values in Each Column")
```
...
test$ever_married<-recode(test$ever_married,Yes="1",No="0")
...
```{r bmi distribution,fig.height=4,fig.width=4,fig.align='center'}
...
```{r stroke across gender, fig.height=4, fig.width=4, warning=TRUE,fig.align='center'}
...
```{r stroke and work type,fig.height=4,fig.width=4,fig.align='center'}
...
```{r,fig.height=4,fig.width=4,message=FALSE,fig.align='center'}
...
```{r kNN demo illustration,fig.height=4,fig.width=4,fig.align='center'}
...
$$Deviance + \lambda
\boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}=-2 log ( f( Y | \mathbf{\boldsymbol{\hat{\beta}}} ) )+\lambda\boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}$$
...
ridgePlot <- function(ridge.mod, metric, title){
  ...
  if(metric == "Deviance"){
  g1 <- ggplot( data.frame(ridge.mod$lambda), aes( x = ridge.mod$lambda, y = sqrt(ridge.mod$cvm))) + geom_point() + geom_vline( xintercept = smallestLambda, col = "blue" , lty = 2 ) +  labs(caption = paste(" Smallest Deviance at lambda = ", smallestLambda), title = title, y = "Deviance", x = "Tuning Parameter")
...
ridgePlot(ridge.modFind_Lambda, metric = "Deviance" , title ="Figure 4.2.1 Change of Deviance vs Lambda (Ridge)" )
```
...
knitr::kable(table("Prediction" = unadjusted_pred_ridge,"Actual"= test$stroke),caption = "Confusion Matrix for Prediction of Stroke Via Ridge (Threshold Unadjusted)")
```
...
```{r ROCAUC ridge, fig.height=4, fig.width=4, message=FALSE, warning=FALSE,fig.align='center'}
...
fancyRpartPlot(sample_tree,sub="Figure 5.2.1 Sample Classification Tree (Only Use 6 Features)",cex=0.7)
```
...
```{r bagged prediction, include=FALSE}
...
Maximum Likelihood Estimation, https://sta279-s22.github.io/slides/lecture_6.html.
