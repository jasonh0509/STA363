...
$$Deviance + \lambda
\boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}=(-2log(L(\hat{\boldsymbol{\beta}})))+\lambda\boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}$$

where the log(L) represents the log likelihood. The likelihood is defined as the probability of the observed data, given that we assume a certain model and certain values for the parameteres that define the model(Evans).
