...
Based on Figure 1 and Figure 3, we could see that the distribution of the body mass seems unimodal with a slight skew to the right. No outlier was found based on these two graphs. 
...
knitr::kable(table(penguins$island, penguins$species))
```
...
ggplot(penguinsClean, aes(x= bill_depth_mm , y = body_mass_g, color = island,pch=species)) + geom_point()+
  labs(title = "Relationship Between penguins Bill Depth and Body Mass (by species and islands)",x = "Bill Depth (in mm)",y = "Body Mass (in grams)")
...
```{r LSLR graphs}
ggplot(penguinsClean, aes(x=flipper_length_mm, y = body_mass_g, color = species))+stat_smooth(method = "lm", formula = y ~ x, size = 1, se = FALSE) + geom_point()+facet_wrap( ~ species, ncol=2)+labs(title = "Relationship Between penguins Bill Depth and Body Mass(by species)",x = "Flipper Length(in mm)",y = "Body Mass(in grams)")
```
