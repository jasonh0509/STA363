...
The test data set is smaller than the training data set because we would need more data points to train the model in order to obtain better model for better prediction/association.
...
The sensitivity of the classification method is 0.185. We prefer methods to have a larer sensitivity.
...
We believe is phenomenon happened because as k increases, there are more points included in the nearest-neighbor range, which increased the error of prediction, in other words, increased the chance of having spam email label as non_spam email. 
...
I don't feel comfortable claiming that the shape of the condition is satisfied, because the linear approach does not effer a good fit to the relationship between predictor Line Breaks and log odds of being spam email.
...
The empirical logit plots using log line breaks is more lineary. Based on the plots, log line breaks wil be used as a feature in the model.
...
$$log\left(\frac{\hat{\pi}_i}{1-\hat{\pi}_i}\right)=\beta_0 + \beta_1log(line\_breaks_i) + \beta_2log(num\_char_i)$$
...
table("Prediction"= predict.Y,"Actual"=testing$spam)
```
...
$Specificity=\frac{808}{808}\approx1=100\%$

The specificity of this classification approach is 1.
...
The FNPR of this classification approach $FNPR=\frac{90}{808+90}\approx0.1=10\% $
