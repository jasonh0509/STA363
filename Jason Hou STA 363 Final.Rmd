...
```{r show where NA is,fig.cap="Graph of NA in Each Column"}
...
```{r stroke class,fig.cap="Classes of Stroke"}
...
```{r bmi distribution,fig.cap="Distribution of BMI"}
...
```{r stroke across gender,fig.cap="Stroke Event Across Gender"}
...
```{r stroke and work type,fig.cap="Stroke Event Across Work Type"}
...
We can see from Figure 5 that their are more patients who had stroke that has private as their work type.

```{r,fig.cap="Distribution of Average Glucose Level"}
ggplot(data=test,aes(x=avg_glucose_level))+
  geom_boxplot()
```

We can see from Figure 6 that the distribution of average glucose level of patients is rightly skewed.

```{r,fig.cap="Scatterplot of Patients' Age vs BMI"}
...
```{r,fig.cap="Scatterplot of Blood Glucose Level vs BMI"}
...
```{r,fig.cap="Age vs BMI Plot Respect to Stroke Status"}
...
```{r,fig.cap="Sample Classification Tree (Only Use 6 features)"}
