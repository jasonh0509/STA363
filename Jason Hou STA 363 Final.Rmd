...
We can see from Figure 2.8 that patients' BMI and their average blood glucose level may have weak correlation.
...
We can see from the Figure 4.1.1 that correlations present between the numeric features in the data set, indicating that multicollinearity exists across features, therefore utilizing penalized regression models is appropriate for the data set.
...
We can see from Figure 4.2.1 and Table 6 that the lowest deviance(0.69) occurred when the model is using $\lambda=0.0182421$ as the tuning parameter of model. Therefore, we select $\lambda = 0.0182421$ and fit the the final logistic regression regression with ridge regularization.
...
We can see from Table 9 that threshold = 0.191465 provides us a balance between sensitivity and specificity of the prediction model, since it has the largest largest geometric mean that indicate the ideal balance between sensitivity and specificity of the prediction model.
...
Below is the confusion matrix of prediction on the set of data that is not been used for growing each tree in the forest, respectively. This is named as the "Out of Bag" data, abbreviated as "OOB" data.
