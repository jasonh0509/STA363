...
```{r correlation plot for numeric features, eval=FALSE, include=FALSE}
M<-cor(test[,c(2,8,9)])
...
Figure 2.7 and 2.8 from the EDA section indicates that some therefore utilizing penalized regression models is appropriate for the data set.
...
$$Deviance + \lambda
\boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}=(-2log(L(\boldsymbol{\beta})))+\lambda\boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}=\sum\limits_{i=1}^nY_ilog(\frac{e^\boldsymbol{\beta}}{1+e^\boldsymbol{\beta}})+\sum\limits_{i=1}^n(1-Y_i)log(\frac{e^\boldsymbol{\beta}}{1+e^\boldsymbol{\beta}})+\lambda\boldsymbol{\hat{\beta}^\mathbf{T}\mathbf{\hat{\beta}}}$$
